...
export default class MessageHandler {
    ...
    async messageFetch(rawMessage) {
        ...
        if (botConfigs.getConfig('debug') == true) console.log(`got new message : ${JSON.stringify(message)}\nfrom : ${pushName}`)
        ...
        if (!text) return null
        if (botConfigs.getConfig('debug') == true) console.log(`got new message after fetch : ${text}\nfrom : ${pushName}`)
        return {
            remoteJid,
            lid,
            pushName,
